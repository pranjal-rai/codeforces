...
int main()
{
    ...
    cin.tie(NULL);
    LL t,sx,sy,ex,ey,l,i,diffx,diffy,counte=0,counts=0,countn=0,countw=0;
    string s;
    cin >>t>>sx>>sy>>ex>>ey;
    cin >>s;
    l=s.length();
    ...
    for(i=0;i<l;i++)
    {
        if(counte==0&&countw==0&&countn==0&&counts==0)
            break;
        if(s[i]=='S'&&counts!=0)
            counts--;
        else if(s[i]=='E'&&counte!=0)
            counte--;
        else if(s[i]=='W'&&countw!=0)
            countw--;
        else if(s[i]=='N'&&countn!=0)
            countn--;
    }
    if(i!=l||(counte==0&&countw==0&&countn==0&&counts==0))
    cout <<i<<"\n";
    else 
        cout <<"-1\n";
    return 0;
