...
{
    FastIO(); 
    LL d1,d2,x,y,n,x1,y1,x2,y2,i,j,ans=LLONG_MAX,r1,r2;
    VPLL v;
    cin >>n>>x1>>y1>>x2>>y2;
    for(i=0;i<n;i++)
    {
    ...
    {
        if(i!=n)
        r1=(v[i].F-x1)*(v[i].F-x1)+(v[i].S-y1)*(v[i].S-y1);
        else{
            r1=0;
        }
            r2=0;
        for(j=0;j<n;j++)
        {
            d1=(v[j].F-x1)*(v[j].F-x1)+(v[j].S-y1)*(v[j].S-y1);
            d2=(v[j].F-x2)*(v[j].F-x2)+(v[j].S-y2)*(v[j].S-y2);
            if(d1>r1&&d2>r2)
                r2=d2;
        }
        ans=min(r1+r2,ans);
